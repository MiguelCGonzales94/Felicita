...
const CAMPO_A_COLUMNA_VENTAS: Record<string, string> = {
  tipo_cp: 'tipo', fecha_emision: 'fecha', serie_cp: 'serie', nro_cp: 'numero',
  nro_doc_identidad: 'ruc', razon_social: 'contraparte',
  bi_gravada: 'base_gravada', bi_no_gravada: 'base_no_gravada',
  exportacion: 'exportacion', igv_ipm: 'igv', total_cp: 'total',
  // Campos adicionales para detalle completo
  fecha_vencimiento: 'fecha_vencimiento', nro_orden_cp: 'nro_orden',
  tipo_cambio: 'tipo_cambio', divisa: 'divisa', nro_doc_referencia: 'doc_referencia',
}
const CAMPO_A_COLUMNA_COMPRAS: Record<string, string> = {
  tipo_cp: 'tipo', fecha_emision: 'fecha', serie_cp: 'serie', nro_cp: 'numero',
  nro_doc_identidad: 'ruc', razon_social: 'contraparte',
  bi_gravado_dg: 'base_gravada', bi_no_gravado_dg: 'base_no_gravada',
  igv_ipm_dg: 'igv', total_cp: 'total',
  // Campos adicionales para detalle completo
  fecha_vencimiento: 'fecha_vencimiento', nro_orden_cp: 'nro_orden',
  tipo_cambio: 'tipo_cambio', divisa: 'divisa', nro_doc_referencia: 'doc_referencia',
}

const TODAS_COLUMNAS_VENTAS = ['incluido', 'tipo', 'serie', 'numero', 'fecha', 'ruc', 'contraparte',
  'base_gravada', 'base_no_gravada', 'exportacion', 'igv', 'total', 'fecha_vencimiento',
  'nro_orden', 'tipo_cambio', 'divisa', 'doc_referencia']
const TODAS_COLUMNAS_COMPRAS = ['incluido', 'tipo', 'serie', 'numero', 'fecha', 'ruc', 'contraparte',
  'base_gravada', 'base_no_gravada', 'igv', 'total', 'fecha_vencimiento',
  'nro_orden', 'tipo_cambio', 'divisa', 'doc_referencia']

type ItemUnificado = {
  id: number; tipo_comprobante: string; serie: string; numero: string
  fecha_emision: string; ruc: string | null; razon_social: string
  base_gravada: number; base_no_gravada: number; exportacion: number
  igv: number; total: number; incluido: boolean
  // Campos adicionales
  fecha_vencimiento?: string; nro_orden?: string; tipo_cambio?: number
  divisa?: string; doc_referencia?: string
}
...
function mapVenta(v: VentaDetalleItem): ItemUnificado {
  return {
    ...
    base_gravada: Number(v.base_gravada), base_no_gravada: Number(v.base_no_gravada),
    exportacion: Number(v.exportacion),
...
function mapCompra(c: CompraDetalleItem): ItemUnificado {
  return {
    ...
    base_gravada: Number(c.base_gravada), base_no_gravada: Number(c.base_no_gravada),
    exportacion: 0,
...
export default function DetalleComprobantesModal({
  ...
  const [columnasVisibles, setColumnasVisibles] = useState<Set<string>>(new Set(
    tipo === 'ventas' ? TODAS_COLUMNAS_VENTAS : TODAS_COLUMNAS_COMPRAS
  ))

  const esVentas = tipo === 'ventas'
  const todasCols = esVentas ? TODAS_COLUMNAS_VENTAS : TODAS_COLUMNAS_COMPRAS
  ...
  async function cargarCamposConfig() {
    try {
      ...
      visibles.add('incluido') // Siempre mostrar checkbox
      for (const campo of data.campos) {
        if (campo.marcado && mapa[campo.codigo]) visibles.add(mapa[campo.codigo])
      }
      // Siempre mostrar al menos las columnas principales
      visibles.add('tipo'); visibles.add('serie'); visibles.add('numero')
      visibles.add('fecha'); visibles.add('contraparte'); visibles.add('total')
      setColumnasVisibles(visibles)
    } catch { setColumnasVisibles(new Set(todasCols)) }
  }
  ...
  const totales = useMemo(() => {
    ...
    return {
      ...
      baseGravada: inc.reduce((s, i) => s + i.base_gravada, 0),
      baseNoGravada: inc.reduce((s, i) => s + i.base_no_gravada, 0),
      exportacion: inc.reduce((s, i) => s + i.exportacion, 0),
  ...
  // Helper para renderizar columna de importe
  const renderImporte = (valor: number) => formatoSoles(valor)

  return (
    <Modal isOpen={isOpen} onClose={guardando ? () => {} : onClose} title={titulo}
      description="Selecciona qué comprobantes entran al cálculo" size="xl"
      footer={<>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input type="text" value={filtro} onChange={e => setFiltro(e.target.value)}
            placeholder="Buscar por RUC, razón social o número..." className="input pl-9" />
        </div>
            ...
            <table className="w-full text-xs">
              <thead className="bg-gray-50 dark:bg-gray-800 sticky top-0 z-10">
                <tr className="text-gray-600 dark:text-gray-300 border-b">
                  {col('incluido') && <th className="py-2 px-2 text-left w-8"><input type="checkbox" checked={todosMarcados} onChange={() => toggleTodos(!todosMarcados)} className="rounded" /></th>}
                  {col('tipo') && <th className="py-2 px-2 text-left font-semibold">Tipo</th>}
                  {col('serie') && <th className="py-2 px-2 text-left font-semibold">Serie</th>}
                  {col('numero') && <th className="py-2 px-2 text-left font-semibold">Número</th>}
                  {col('fecha') && <th className="py-2 px-2 text-left font-semibold">Fecha</th>}
                  {col('ruc') && <th className="py-2 px-2 text-left font-semibold">RUC</th>}
                  {col('contraparte') && <th className="py-2 px-2 text-left font-semibold">{esVentas ? 'Cliente' : 'Proveedor'}</th>}
                  {col('base_gravada') && <th className="py-2 px-2 text-right font-semibold">Base Grab.</th>}
                  {col('base_no_gravada') && <th className="py-2 px-2 text-right font-semibold">Base No Grab.</th>}
                  {esVentas && col('exportacion') && <th className="py-2 px-2 text-right font-semibold">Export.</th>}
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {filtrados.map(i => (
                  <tr key={i.id} onClick={() => toggle(i.id)} className={`transition-colors ${editable ? 'cursor-pointer' : ''} ${i.incluido ? 'hover:bg-brand-50/50 dark:hover:bg-brand-900/20' : 'bg-gray-50/50 dark:bg-gray-800/50 text-gray-400'}`}>
                    {col('incluido') && <td className="py-2 px-2"><input type="checkbox" checked={i.incluido} onChange={() => toggle(i.id)} onClick={e => e.stopPropagation()} disabled={!editable} className="rounded" /></td>}
                    {col('tipo') && <td className="py-2 px-2 font-medium">{TIPO_COMPROBANTE_LABEL[i.tipo_comprobante] || i.tipo_comprobante}</td>}
                    {col('serie') && <td className="py-2 px-2 font-mono">{i.serie}</td>}
                    {col('numero') && <td className="py-2 px-2 font-mono">{i.numero}</td>}
                    {col('fecha') && <td className="py-2 px-2">{new Date(i.fecha_emision).toLocaleDateString('es-PE')}</td>}
                    {col('ruc') && <td className="py-2 px-2 font-mono text-[10px]">{i.ruc || '-'}</td>}
                    {col('contraparte') && <td className="py-2 px-2 max-w-[150px] truncate"><div className="truncate">{i.razon_social}</div></td>}
                    {col('base_gravada') && <td className="py-2 px-2 text-right font-mono">{renderImporte(i.base_gravada)}</td>}
                    {col('base_no_gravada') && <td className="py-2 px-2 text-right font-mono">{renderImporte(i.base_no_gravada)}</td>}
                    {esVentas && col('exportacion') && <td className="py-2 px-2 text-right font-mono">{renderImporte(i.exportacion)}</td>}
                    {col('igv') && <td className="py-2 px-2 text-right font-mono">{renderImporte(i.igv)}</td>}
                    {col('total') && <td className="py-2 px-2 text-right font-mono font-semibold">{renderImporte(i.total)}</td>}
                  </tr>
                ))}
              </tbody>
              <tfoot className="bg-gray-50 dark:bg-gray-800 sticky bottom-0">
                <tr className="border-t-2 font-semibold text-gray-800 dark:text-gray-200">
                  <td colSpan={col('contraparte') ? 6 : 5} className="py-2 px-2 text-right text-[11px] uppercase tracking-wider text-gray-600">
                  ...
                  {col('base_gravada') && <td className="py-2 px-2 text-right font-mono">{renderImporte(totales.baseGravada)}</td>}
                  {col('base_no_gravada') && <td className="py-2 px-2 text-right font-mono">{renderImporte(totales.baseNoGravada)}</td>}
                  {esVentas && col('exportacion') && <td className="py-2 px-2 text-right font-mono">{renderImporte(totales.exportacion)}</td>}
                  {col('igv') && <td className="py-2 px-2 text-right font-mono">{renderImporte(totales.igv)}</td>}
                  {col('total') && <td className="py-2 px-2 text-right font-mono">{renderImporte(totales.totalImporte)}</td>}
                </tr>
              </tfoot>
            </table>
          </div>
