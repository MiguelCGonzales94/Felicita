# ============================================================
#  FELICITA - Entrega 2 Parte B: Frontend Config Tributaria
#  Ruta /empresas/:id/tributario con tabs + sidebar item
#  Uso: .\entrega2b_config_tributaria_frontend.ps1
# ============================================================

Write-Host ""
Write-Host "Entrega 2 - Parte B: Frontend Configuracion Tributaria" -ForegroundColor Cyan
Write-Host ""
...
# ============================================================
# 1. types/configuracionTributaria.ts
# ============================================================

Write-Host "Creando tipos..." -ForegroundColor Yellow
...
'@ | Set-Content "frontend/src/types/configuracionTributaria.ts"

Write-Host "  [OK] types/configuracionTributaria.ts" -ForegroundColor Green

# ============================================================
# 2. services/configuracionTributariaService.ts
# ============================================================
...
'@ | Set-Content "frontend/src/services/configuracionTributariaService.ts"

Write-Host "  [OK] services/configuracionTributariaService.ts" -ForegroundColor Green

# ============================================================
# 3. pages/empresa/Tributario.tsx - pagina principal con tabs
# ============================================================

Write-Host "Creando pagina Tributario.tsx..." -ForegroundColor Yellow
...
'@ | Set-Content "frontend/src/pages/empresa/Tributario.tsx"

Write-Host "  [OK] pages/empresa/Tributario.tsx" -ForegroundColor Green

# ============================================================
# 4. Agregar item "Tributario" al sidebar + ruta en App.tsx
# ============================================================

Write-Host ""
Write-Host "Agregando item al sidebar..." -ForegroundColor Yellow

$sidebarPath = "frontend/src/components/EmpresaSidebar.tsx"
$sidebarContent = Get-Content $sidebarPath -Raw

# Agregar import de Scale si no existe
if ($sidebarContent -notmatch "Scale") {
    $sidebarContent = $sidebarContent -replace `
        "(import \{[^}]*Settings)",
        "`$1, Scale"
}

# Agregar item al NAV_CONFIG (justo despues de Configuracion)
if ($sidebarContent -notmatch "tributario") {
    $sidebarContent = $sidebarContent -replace `
        "(\{ to: .*/configuracion.*label: 'Configuracion'.*icon: Settings \},?)",
        "`$1`r`n    { to: ``/empresas/`${empresa.id}/tributario``, label: 'Tributario', icon: Scale },"
}

Set-Content $sidebarPath $sidebarContent -NoNewline
Write-Host "  [OK] EmpresaSidebar.tsx - item 'Tributario' agregado" -ForegroundColor Green
...
Write-Host ""
Write-Host "Agregando ruta en App.tsx..." -ForegroundColor Yellow

$appPath = "frontend/src/App.tsx"
$appContent = Get-Content $appPath -Raw

# Agregar import de Tributario
if ($appContent -notmatch "Tributario") {
    # Agregar import
    $appContent = $appContent -replace `
        "(import Configuracion from.*empresa/Configuracion.*)",
        "`$1`r`nconst Tributario = lazy(() => import('./pages/empresa/Tributario'))"

    # Agregar ruta (dentro de las rutas de empresa)
    $appContent = $appContent -replace `
        '(<Route path="configuracion"[^/]*/\s*>)',
        "`$1`r`n                <Route path=""tributario"" element={<Tributario />} />"
}

Set-Content $appPath $appContent -NoNewline
Write-Host "  [OK] App.tsx - ruta /empresas/:id/tributario agregada" -ForegroundColor Green
...
Write-Host "  [NEW] frontend/src/types/configuracionTributaria.ts" -ForegroundColor Green
Write-Host "  [NEW] frontend/src/services/configuracionTributariaService.ts" -ForegroundColor Green
Write-Host "  [NEW] frontend/src/pages/empresa/Tributario.tsx" -ForegroundColor Green
Write-Host "  [MOD] frontend/src/components/EmpresaSidebar.tsx (item Scale)" -ForegroundColor Green
Write-Host "  [MOD] frontend/src/App.tsx (ruta tributario)" -ForegroundColor Green
Write-Host ""
Write-Host "COMO PROBAR:" -ForegroundColor Cyan
Write-Host "  1. Vite deberia recargar solo" -ForegroundColor Yellow
Write-Host "  2. Login -> Empresa Gamma SA -> sidebar -> 'Tributario' (icono balanza)" -ForegroundColor Yellow
Write-Host "  3. Tab 'Valores legales':" -ForegroundColor Yellow
Write-Host "     - Todos los valores bloqueados con candado" -ForegroundColor Gray
Write-Host "     - Click en candado -> se desbloquea con advertencia naranja" -ForegroundColor Gray
Write-Host "     - Editar UIT a 5500 -> badge 'Modificado' aparece" -ForegroundColor Gray
Write-Host "     - Guardar -> toast verde" -ForegroundColor Gray
Write-Host "     - Restaurar defaults -> vuelve a 5350" -ForegroundColor Gray
Write-Host "  4. Tab 'Campos SIRE':" -ForegroundColor Yellow
Write-Host "     - Dos columnas: Ventas (RVIE) y Compras (RCE)" -ForegroundColor Gray
Write-Host "     - Campos obligatorios con candado SUNAT no se pueden desmarcar" -ForegroundColor Gray
Write-Host "     - Boton 'Seleccionar todos' / 'Solo obligatorios'" -ForegroundColor Gray
Write-Host "     - Seccion CLU colapsable al fondo" -ForegroundColor Gray
Write-Host "     - Guardar por separado (RVIE y RCE)" -ForegroundColor Gray
Write-Host ""
Write-Host "NOTA: si el regex de EmpresaSidebar.tsx o App.tsx no matchea tu" -ForegroundColor Yellow
Write-Host "version actual, edita manualmente:" -ForegroundColor Yellow
Write-Host "  - En EmpresaSidebar: agrega { to: tributario, icon: Scale } en NAV_CONFIG" -ForegroundColor Gray
Write-Host "  - En App.tsx: agrega <Route path='tributario' element={<Tributario />} />" -ForegroundColor Gray
Write-Host ""
